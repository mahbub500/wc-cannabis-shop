...
( function () {
    ...
    // Filter state — seed category from the shortcode attribute so "Clear All"
    // restores to the original filtered view rather than showing every product.
    const state = {
        category   : shop.dataset.defaultCategory || '',
        strain     : '',
        search     : '',
        sort       : 'menu_order',
        paged      : 1,
     ...
     * We compare how many cards are currently in the DOM against the server total.
     * Button is hidden when everything is already loaded.
     */
    function updateLoadMore( serverTotal ) {
        if ( ! loadMore ) return;
        const loaded = grid.querySelectorAll( '.wccs-product-card' ).length;
        loadMore.style.display = loaded < serverTotal ? 'block' : 'none';
    }
    ...
    async function fetchProducts( append = false ) {
        ...
        try {
            ...
            if ( data.success ) {
                ...
                if ( append ) {
                    // Preserve the grid and its --wccs-cols variable; only add new cards.
                    if ( html.trim() ) {
                ...
                // Re-evaluate button after every fetch (both replace and append).
    ...
    if ( loadMore ) {
        loadMore.addEventListener( 'click', () => {
            state.paged++;
            fetchProducts( true ); // append = true → grid is NOT cleared
        } );
    }
    ...
    function resetAllFilters() {
        // Restore the shortcode's original category, not blank
        state.category  = shop.dataset.defaultCategory || '';
        state.strain    = '';
        state.search    = '';
    ...
    /* ─── Seed load-more state from server-rendered count ─── */
    // The button's display is already set correctly by PHP via $show_load_more.
    // This call keeps the JS state in sync so subsequent filter changes are correct.
    if ( countEl && loadMore ) {
        const serverTotal = parseInt( countEl.textContent, 10 ) || 0;
        updateLoadMore( serverTotal );
    }
 ...
 * Requires jQuery (loaded by WooCommerce)
 */
( function ( $ ) {
    'use strict';
    ...
    function matchVariationServer( $overlay, attributes, productId ) {
        ...
        attributes.forEach( function ( attr ) {
            ...
            if ( val ) selected[ 'attribute_' + attr.name ] = val;
        } );
...
( function () {
    ...
    async function fetchCartData() {
        try {
            const res  = await fetch( wccs.ajax_url, {
                method: 'POST',
                body: new URLSearchParams( { action: 'wccs_get_cart_data', nonce: wccs.nonce } ),
            } );
            const data = await res.json();
            if ( data.success ) updateCartUI( data.data );
        } catch ( err ) {
    ...
    function updateCartUI( cart ) {
        ...
        cartItems.innerHTML = items.map( item => `
            <div class="wccs-cart-item" data-key="${item.key}">
                <img class="wccs-cart-item-image" src="${item.image}" alt="${item.name}" loading="lazy">
                <div class="wccs-cart-item-info">
                    ...
                    <div class="wccs-cart-item-qty-row">
                        <button class="wccs-qty-btn wccs-qty-minus" data-key="${item.key}" aria-label="Decrease quantity">
                            <svg width="12" height="12" viewBox="0 0 12 2" fill="none"><rect width="12" height="2" rx="1" fill="currentColor"/></svg>
                        </button>
                        ...
                        <button class="wccs-qty-btn wccs-qty-plus" data-key="${item.key}" aria-label="Increase quantity">
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M5 0h2v12H5zM0 5h12v2H0z" fill="currentColor"/></svg>
                        </button>
                    </div>
                </div>
                <div class="wccs-cart-item-price">${item.price}</div>
                <button class="wccs-cart-item-remove" data-key="${item.key}" aria-label="Remove ${item.name}">
    ...
    cartItems.addEventListener( 'click', async e => {
        ...
        try {
            const res  = await fetch( wccs.ajax_url, {
                method: 'POST',
                body: new URLSearchParams( { action: 'wccs_remove_from_cart', nonce: wccs.nonce, cart_key: key } ),
            } );
            const data = await res.json();
            if ( data.success ) {
    ...
    cartItems.addEventListener( 'click', async e => {
        ...
        try {
            const res  = await fetch( wccs.ajax_url, {
                method: 'POST',
                body: new URLSearchParams( { action: 'wccs_update_cart_qty', nonce: wccs.nonce, cart_key: key, quantity: newQty } ),
            } );
            const data = await res.json();
            if ( data.success ) {
                updateCartUI( data.data );
    ...
    /* ── Global cart refresh ── */
    document.body.addEventListener( 'wccs_refresh_cart', fetchCartData );
    document.body.addEventListener( 'wc_fragment_refresh', fetchCartData );

    /* ── Toast ── */
    function showToast( message, type = 'success' ) {
        document.querySelector( '.wccs-toast' )?.remove();
        ...
        requestAnimationFrame( () => toast.classList.add( 'wccs-toast-enter' ) );
        setTimeout( () => {
            toast.classList.replace( 'wccs-toast-enter', 'wccs-toast-leave' );
    ...
    if ( confirmYesBtn ) {
        confirmYesBtn.addEventListener( 'click', async () => {
            ...
            try {
                const res  = await fetch( wccs.ajax_url, {
                    method: 'POST',
                    body: new URLSearchParams( { action: 'wccs_clear_cart', nonce: wccs.nonce } ),
                } );
                const data = await res.json();
                if ( data.success ) {
                    updateCartUI( data.data );
